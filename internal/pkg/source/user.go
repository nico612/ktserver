...
var _ initdb.SubInitializer = (*initUser)(nil)
...
func (i *initUser) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	if err = db.Create(&entities).Error; err != nil {
		return ctx, errors.Wrap(err, model.SysUser{}.TableName()+"表数据初始化失败!")
	}
	next = context.WithValue(ctx, i.InitializerName(), entities)
	authorityEntities, ok := ctx.Value(initAuthority{}.InitializerName()).([]model.SysAuthority)
