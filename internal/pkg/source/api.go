...
// MigrateTable 自动迁移表
func (i *InitApi) MigrateTable(ctx context.Context) (next context.Context, err error) {
...
// DataInserted 判断数据是否已经存在
func (i *InitApi) DataInserted(ctx context.Context) bool {
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return false
	}
	if err := db.Where("path = ? AND method = ?", "/authorityBtn/canRemoveAuthorityBtn", "POST").First(&model.SysApi{}).Error; err != nil {
...
func (i *InitApi) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	netx := context.WithValue(ctx, i.InitializerName(), entities)
