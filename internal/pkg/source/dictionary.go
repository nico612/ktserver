...
func (i *initDict) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	if err = db.Create(&entities).Error; err != nil {
		return ctx, errors.Wrap(err, model.SysDictionary{}.TableName()+"表数据初始化失败!")
	}
...
func (i *initDict) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("type = ?", "bool").First(&model.SysDictionary{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
